...
from mammo_lejepa.download import build_client_session, download_dicom, validate_access
from mammo_lejepa.errors import AuthError, NetworkError, redact_secrets
...
from mammo_lejepa.storage import append_record, quarantine
from mammo_lejepa.worker import process_dicom
...
async def run_pipeline(
    ...
    *,
    ...
    PNG → registro → borrado del DICOM (D-08), impone la contrapresión de la
    cola acotada (FR-016, FR-019) y devuelve el resumen de la ejecución.
    `on_result`, si se da, se invoca de forma síncrona tras cada imagen
    resuelta (FR-029: progreso en consola). `shutdown_event`, si se marca
    ...
    termina la ejecución con `exit_reason="interrupted"` (T037)."""
    started_at = _now_iso()
...
async def _consume(
    ...
    while True:
        ...
        append_record(record, config.images_jsonl_path)
...
def _redact_outcome(
    ...
    """Sanea `error_message` antes de que llegue a `append_record` (FR-031,
    T053). `worker.py` nunca ve las credenciales (procesa DICOM ya
    descargados, sin red), así que esto es una segunda capa de defensa por si
    una librería externa llegase a incluir texto inesperado en una
    excepción."""
