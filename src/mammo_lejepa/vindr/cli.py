...
from mammo_lejepa.resume import latest_records_by_image, pending_tasks
...
from mammo_lejepa.vindr.catalog import (
    consolidate as consolidate_catalog,
)
from mammo_lejepa.vindr.catalog import (
    parse_jsonl_records,
...
from mammo_lejepa.vindr.config import (
    PhysioNetCredentials,
    PipelineConfig,
    load_credentials,
)
from mammo_lejepa.vindr.download import build_client_session, validate_access
from mammo_lejepa.vindr.errors import AuthError
from mammo_lejepa.vindr.manifest import (
    batch_by_study,
    build_manifest,
...
from mammo_lejepa.vindr.pipeline import run_pipeline

app = typer.Typer(
    help="mammo-etl: ETL de VinDr-Mammo hacia recortes mamarios catalogados, "
    "con hallazgos remapeados para evaluación de localización.",
...
@app.command()
def doctor() -> None:
    """Comprueba el entorno sin descargar ninguna imagen (FR-022): CSV,
    decodificador JPEG 2000, permisos de escritura, espacio libre frente al
    techo de disco calculado, y validez de la cookie de PhysioNet — cada
    comprobación en su propia línea, para que un CSV ausente nunca se
    confunda con una cookie caducada (SC-005)."""
    ...
    try:
        ...
        # Ni usuario, ni contraseña, ni cookie se imprimen jamás, sólo la
        # confirmación de que se cargaron (FR-026).
...
def resume() -> None:
    ...
    realmente) y descarga únicamente lo pendiente (FR-023)."""
    config = PipelineConfig()
    credentials = _require_credentials()
...
def _warn_metadata_discrepancies(
    ...
    if only_in_metadata or only_in_annotations:
        console.print(
            f"[yellow]Aviso:[/yellow] {only_in_metadata} imagen(es) en "
...
def _execute_pipeline(
    ...
    y al final, lanza el pipeline con barra de progreso y cierre ordenado ante
    `SIGINT`/`SIGTERM`, y traduce `AuthError` en un mensaje que explica cómo
    renovar la cookie sin perder el trabajo ya hecho (FR-022, FR-023)."""
    ...
    append_run_summary(start_marker, config.runs_jsonl_path)  # pyright: ignore[reportArgumentType]
    ...
    append_run_summary(summary, config.runs_jsonl_path)  # pyright: ignore[reportArgumentType]
...
def retry(
    ...
    """Reintenta selectivamente las imágenes marcadas como fallidas (FR-024).
...
def consolidate() -> None:
    ...
    `images.parquet` y `findings.parquet` (FR-018/FR-016), verifica la
    ausencia de fuga de partición por estudio (FR-021) y la coherencia del
    catálogo (FR-019)."""
    config = PipelineConfig()
    ...
    try:
        ...
        raise typer.Exit(code=1) from error
    console.print(
        "[green]Partición verificada:[/green] ningún estudio aparece en más de "
        "un split (FR-021, SC-003)."
    )
...
def inspect(
    ...
    detectada y recorte con los hallazgos remapeados superpuestos (FR-028,
    SC-004). El DICOM de cada imagen inspeccionada ya se borró tras
    procesarse; se vuelve a descargar temporalmente sólo para esta inspección
    y se elimina de nuevo al terminar."""
    config = PipelineConfig()
...
async def _collect_inspection_panels(
    ...
    from mammo_lejepa.vindr.download import download_dicom
        ...
        for row in ok_images.iter_rows(named=True):
            ...
            try:
                ...
                crop = CajaMamaria(
                    ...
                    threshold=row["crop_otsu_threshold"],
                    image_height=row["source_height"],
                    image_width=row["source_width"],
