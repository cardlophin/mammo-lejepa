...
from mammo_lejepa.vindr.models import DicomPayload
...
def read_dicom(path: Path) -> DicomPayload:
    ...
    `DecodeError` (compartido con Mammo-Bench: el concepto es el mismo)
    nombrando la sintaxis de transferencia y el paquete ausente, en lugar de
    dejar pasar un `AttributeError` opaco (FR-009, edge case de sintaxis sin
    decodificador)."""
