...
from mammo_lejepa.vindr.config import PipelineConfig
from mammo_lejepa.vindr.dicom_io import read_dicom
from mammo_lejepa.vindr.errors import classify_exception
from mammo_lejepa.vindr.models import ImageTask, ProcessOutcome
...
def process_dicom(
    ...
    """Encadena lectura, ventana, normalización, detección, recorte (a
    resolución nativa, FR-010) y escritura del PNG. Nunca lanza excepciones al
    proceso padre: cualquier fallo se captura y se devuelve como un
    `ProcessOutcome` fallido con su categoría (contrato de `worker.py`)."""
    start = perf_counter()
