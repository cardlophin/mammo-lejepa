...
SRC_DIR = Path(__file__).resolve().parent.parent.parent / "src" / "mammo_lejepa" / "ssl"
...
def test_pure_module_does_not_import_io_libraries(module_name: str) -> None:
    ...
    assert not forbidden_found, (
        f"ssl/{module_name}.py importa {forbidden_found}, prohibido para "
        "módulos puros (constitución, principio I)"
    )
