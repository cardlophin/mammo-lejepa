...
def pipeline_config(tmp_path: Path) -> PipelineConfig:
    """`PipelineConfig` de prueba: rutas bajo un directorio temporal, sin tocar
    `data/` real."""
    return PipelineConfig(data_dir=tmp_path / "vindr-mammo")


@pytest.fixture
def credentials() -> PhysioNetCredentials:
    """Credenciales de prueba: nunca las reales, no hace falta `.env`."""
    return PhysioNetCredentials(
        username="test_user", password="test_pass", session_id="test_session_id"
    )
